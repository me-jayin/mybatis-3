...
/**
 * 该类用于注册 TypeHandler，有以下方式注册 TypeHandler：
 * 1.xml中的 <typeHandler /> 标签
 * 2.使用 <package /> 标签，使用该标签时，有以下两种方式指定 javaType 和 jdbcType
 * a.默认有mybatis自动识别 javaType
 * b.使用 MappedTypes 指定 javaType，使用 MappedJdbcTypes 指定 jdbcType
 */
public final class TypeHandlerRegistry {
  ...
  public TypeHandlerRegistry(Configuration configuration) {
    // 添加默认未识别到的 TypeHandler
    this.unknownTypeHandler = new UnknownTypeHandler(configuration);

    // 注册预制的 TypeHandler，还有在 org.apache.ibatis.session.Configuration.Configuration() 构造器中也会进行预注册
  ...
  /////////////////////// TypeHandler重点
  /**
   * 注意这里，在获取实例时，会先尝试 通过接受一个class类型的构造器 来构建 TypeHandler ，也就是会传入 javaType；
   * 因此，如果需要一个通用的 TypeHandler，我们可以具体 需要的类型 由构造器参数传入，可参考 {@link xyz.me4cxy.typehandler.CustomJsonTypeHandler}
   */
  public <T> TypeHandler<T> getInstance(Class<?> javaTypeClass, Class<?> typeHandlerClass) {
    if (javaTypeClass != null) {
      try {
        // 尝试获取一个接受Class类的TypeHandler构造器，如果存在会将当前目标的JavaType传入来创建实例
